class Pathfinder extends Phaser.Scene {
    ...
    generateGameConstraints()
    {
        this.gameConstraintsArray = [];
        for (let y = 0; y < this.TILEHEIGHT; y++)
        {
            this.gameConstraintsArray[y] = [];
            for (let x = 0; x < this.TILEWIDTH; x++)
            {
                this.gameConstraintsArray[y][x] = [];
            }
        }

        // ----- WHEELBARROW ----- //
        if (window.wheelbarrowPosX != undefined)
        {
            console.log("wheelbarrowX: " + window.wheelbarrowPosX);
            console.log("wheelbarrowY: " + window.wheelbarrowPosY);

            this.gameConstraintsArray[window.wheelbarrowPosY][window.wheelbarrowPosX] = 58;
        }

        // ----- MUSHROOM ----- //
        if (window.mushroomPosX == undefined)
        {
            let mushroomValidities = [];
            for (let y = 0; y < this.TILEHEIGHT; y++)
            {
                mushroomValidities[y] = [];
                for (let x = 0; x < this.TILEWIDTH; x++)
                {
                    if (this.map.hasTileAt(x, y, this.treesLayer))
                    {
                        mushroomValidities[y][x] = false;
                    }
                    else
                    {
                        mushroomValidities[y][x] = true;
                    }
                }
            }
            window.mushroomValidities = mushroomValidities;
        }
        else
        {
            console.log("mushroomX: " + window.mushroomPosX);
            console.log("mushroomY: " + window.mushroomPosY);

            this.gameConstraintsArray[window.mushroomPosY][window.mushroomPosX] = 30;
        }

        this.gameConstraintsMap = this.make.tilemap({
            data: this.gameConstraintsArray,
            tileWidth: this.TILESIZE,
            tileHeight: this.TILESIZE
        })
        
        this.gameConstraintsLayer = this.gameConstraintsMap.createLayer(0, this.tileset, 0, 0);
    }
